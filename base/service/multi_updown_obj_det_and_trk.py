...
from base.service.trackbleobject import TrackableObject
from base.service.utils.general import set_logging
from base.service.models.common import DetectMultiBackend
from base.service.utils.dataloaders import IMG_FORMATS, VID_FORMATS, LoadImages, LoadStreams
from base.service.utils.general import (LOGGER, Profile, check_file, check_img_size,
                            check_imshow, check_requirements, colorstr, cv2,
                           increment_path, non_max_suppression, print_args,
                            scale_coords, strip_optimizer, xyxy2xywh)
from base.service.utils.plots import Annotator, colors, save_one_box
from base.service.utils.torch_utils import select_device, time_sync
...
@torch.no_grad()
def detect(source,weights=r'base/service/yolov5s.pt',
        data=r'base/service/data/coco128.yaml',
    ...
    listDet = [ 'bicycle', 'car', 'motorcycle']
    rects = []
    labelObj = []
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
                            ...
                            if not to.counted:
                                ...
                                if direction < 0 and middle > y_c > height // 2.3:
                                    if (idx == 2):
                                        totalUpCar += 1
                                        to.counted = True
                                        #occ_car = totalDownCar - totalUpCar
                                    elif (idx == 3 or idx == 1):
                                        totalUpMotor += 1
                                        to.counted = True
                                        #occ_motorcycle = totalDownMotor - totalUpMotor
                                    elif (idx == 5):
                                        totalUpBus += 1
                                        to.counted = True
                                        #occ_bus = totalDownBus - totalUpBus
                                    elif (idx == 7):
                                        totalUpTruck += 1
                                        to.counted = True
                                        #occ_truck = totalDownTruck - totalUpTruck
            ...
            cv2.putText(im0, 'Entering cars : ' + str(totalDownCar), (int(width * 0.6), int(height * 0.15)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            cv2.putText(im0, 'Entering motorcycle : ' + str(totalDownMotor), (int(width * 0.6), int(height * 0.2)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            cv2.putText(im0, 'Entering bus : ' + str(totalDownBus), (int(width * 0.6), int(height * 0.25)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            cv2.putText(im0, 'Entering truck : ' + str(totalDownTruck), (int(width * 0.6), int(height * 0.3)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            cv2.putText(im0, 'Total Entry Count : ' + str(totalDown), (int(width * 0.6), int(height * 0.35)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)


            cv2.putText(im0, 'Exiting cars : ' + str(totalUpCar), (int(width * 0.02), int(height * 0.15)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            cv2.putText(im0, 'Exiting motorcycles : ' + str(totalUpMotor), (int(width * 0.02), int(height * 0.2)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            cv2.putText(im0, 'Exiting bus : ' + str(totalUpBus), (int(width * 0.02), int(height * 0.25)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            cv2.putText(im0, 'Exiting truck : ' + str(totalUpTruck), (int(width * 0.02), int(height * 0.3)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
            cv2.putText(im0, 'Total Exit Count : ' + str(totalUp), (int(width * 0.02), int(height * 0.35)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)


            '''cv2.putText(im0, 'Occupied truck space : ' + str(occ_truck), (int(width * 0.02), int(height * 0.35)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255,0,0), 2)
            cv2.putText(im0, 'Occupied car space : ' + str(occ_car), (int(width * 0.02), int(height * 0.4)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
            cv2.putText(im0, 'Occupied bus space : ' + str(occ_bus), (int(width * 0.02), int(height * 0.45)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
            cv2.putText(im0, 'Occupied motorcycle space : ' + str(occ_motorcycle), (int(width * 0.02), int(height * 0.5)),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)'''
            if view_img:
    ...
    if vid_cap:
        ...
        vid_cap.release()
    return video_save_path,totalDown,totalUp